...
### Minh chứng pipeline fail do unit test

Để chứng minh CI có thể phát hiện và chặn code lỗi, em tạo branch riêng:

```text
demo/intentional-test-failure
```

Trong branch này em thêm một test cố tình so sánh hai giá trị khác nhau:
...
Sau đó em push branch và mở Pull Request vào `main`:

- Pull Request: `#4` — <https://github.com/chiendz11/demo-app-ci/pull/4>
- Workflow run: Run `#5` — <https://github.com/chiendz11/demo-app-ci/actions/runs/27972673528>

Kết quả pipeline:

| Job | Kết quả | Giải thích |
| :--- | :--- | :--- |
| `Lint` | Pass | Source code vẫn đúng rule ESLint. |
| `Unit test` | Fail | Test cố tình tạo ra `AssertionError`: giá trị `actual` khác `expected`. |
| `Build, scan, SBOM, and optionally push image` | Skipped | Job này phụ thuộc vào test bằng `needs: test`, nên không chạy khi unit test fail. |

Log chính:

```text
Subtest: intentional failure to prove CI works
not ok 1 - intentional failure to prove CI works
...
Kết quả này chứng minh pipeline không chỉ chạy cho có. Khi test fail, CI trả về trạng thái đỏ và dừng các bước build, scan, tạo SBOM và push image phía sau. PR này chỉ dùng để minh chứng nên không được merge vào `main`.
...
- [x] Có pipeline fail do unit test và chặn các job phía sau.
